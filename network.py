...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        d7 = self.deconv8(c8)
        d6 = self.deconv7(torch.cat([c7, d7], dim=1))
        d5 = self.deconv6(torch.cat([c6, d6], dim=1))
        d4 = self.deconv5(torch.cat([c5, d5], dim=1))
        d3 = self.deconv4(torch.cat([c4, d4], dim=1))
        d2 = self.deconv3(torch.cat([c3, d3], dim=1))
        d1 = self.deconv2(torch.cat([c2, d2], dim=1))
        out = self.deconv1(torch.cat([c1, d1], dim=1))
